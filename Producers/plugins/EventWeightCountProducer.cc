...
class EventWeightCountProducer : public edm::one::EDProducer<edm::one::WatchLuminosityBlocks, edm::EndLuminosityBlockProducer>
{
	public:
...
EventWeightCountProducer::EventWeightCountProducer(const edm::ParameterSet& iConfig)
{
	...
	#endif
	if (iConfig.existsAs<bool>("isMC")) isMC = iConfig.getParameter<bool>("isMC");
	else throw cms::Exception("Configuration")<<"Missing parameter input isMC \n";\

	if (iConfig.existsAs<bool>("countNegWeightsOnly")) countNegWeightsOnly = iConfig.getParameter<bool>("countNegWeightsOnly");
...
void EventWeightCountProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	if (not isMC || not countNegWeightsOnly) nEvents++;
	else
	{
		edm::Handle<GenEventInfoProduct> GenEventInfoHandle;
		iEvent.getByToken(srcGenEventInfoToken_, GenEventInfoHandle);
		if( GenEventInfoHandle->weight() < 0) nEvents++;
	}
	return;
}
