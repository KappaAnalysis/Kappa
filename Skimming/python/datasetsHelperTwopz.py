...
class datasetsHelperTwopz:
	...
	def __init__(self, in_json_file ) :
		...
		self.base_dict = {}
		self.keep_input_json = True # can overwrite the input json
		...
		self.convert_dict_to_nickorder()
	...
	def read_from_jsonfile(self, in_json_filename):
		...
		with open(in_json_filename) as in_json_file:
			self.base_dict = json.load(in_json_file)
	...
	def getTreeInfo(self,nick, globaltag, kappaTag):
		...
		sub_dict = self.base_dict.get(nick, {}) ## to run with unkown nick
		import FWCore.ParameterSet.Config as cms
		return cms.PSet(
		...
		for akt_nick in pre_selected_nicks:
			...
			for test_key in query.keys():
				if type(query[test_key]) in [str, unicode]:
					if not re.match(query[test_key], self.base_dict[akt_nick].get(test_key, "allways_false")) or test_key not in self.base_dict[akt_nick].keys():
						take_nick = False
				elif query[test_key] != self.base_dict[akt_nick][test_key]:
						take_nick = False

			if take_nick: nick_list.append(akt_nick)
	...
	def get_nicks_with_tag(self, tag, tag_values=None, pre_selected_nicks=None):
		nick_list = []
		if pre_selected_nicks is None:
			pre_selected_nicks = self.base_dict.keys()

		for akt_nick in pre_selected_nicks:
			take_nick = False

			if not tag_values and tag in self.base_dict[akt_nick].keys():
				take_nick = True
			else:
	...
	def dataset_diff_base(self, obj_A, obj_B, global_diff = {}, depth = [], test_key = None):
		...
		#~ if type(obj_A) == unicode: obj_A = str(obj_A)
		if type(obj_A) != type(obj_B):
			...
			for key in obj_A.keys():
				if key not in obj_B.keys():
					# if the key is not in b take the hole key as differnces
			...
			print "Wrong type:", type(obj_A), " ", obj_A
