...
class datasethelpertwopz:
  ...
  def dataset_diff_base(self, obj_A, obj_B, global_diff = {}, depth = [], test_key = None):
    ...
    Not the nicesets function so far, but at least it seems to work now. The idea is also to push the depth of the (sub)dict , and only fill the global diff when there is a differnces
   ...
   # print global_diff
    if type(obj_A) != type(obj_B):
      print "Comparing apples with bannas"
