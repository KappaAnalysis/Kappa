...
class DataSetMangerBase:
  ...
  def add_dataset(self,dbs,inputDBS=None,xsec=None,nick_name=None):
    new_entry = {'dbs' : dbs}
...
#  def_input = os.path.join(os.environ.get("CMSSW_BASE"),"src/Kappa/Skimming/data/datasets_test.json")
  parser.add_argument("--input", dest="input", help="input data base (default=%s)"%def_input, default=def_input)
  
  parser.add_argument("--save", dest="save", help="save data base to file. For local storage please make ./filname.json otherwies $CMSSW_BASE/src/Kappa/Skimming/data/filename.json is used ")
  ...
  DSM = DataSetMangerBase(args.input, tag_key=args.tag, tag_values_str=args.tagvalues, query=args.query, nick_regex=args.nicks )
